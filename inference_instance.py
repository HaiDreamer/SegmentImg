...
from model_train_v4_improved import (
...
def main():
    ...
    # Remove padding
    sem_logits = sem_logits[0, :new_h, :new_w]  
    if boundary_logits is not None:
        if boundary_logits.ndim == 4:
            boundary_logits = boundary_logits[0, :new_h, :new_w, 0]  
        else:
            boundary_logits = boundary_logits[0, :new_h, :new_w]  
    
    # Resize back to original size
    sem_probs = tf.nn.softmax(sem_logits, axis=-1).numpy()  
    ...
    if args.use_instance:
        if boundary_logits is None:
            print("  Warning: Boundary logits not available. Cannot perform instance segmentation.")
            ...
            boundary_logits_resized = np.expand_dims(boundary_resized, axis=-1)  
            
            # Convert to logits format for function
            sem_logits_resized = np.log(sem_probs_resized + 1e-8)  
            sem_logits_resized = np.transpose(sem_logits_resized, (2, 0, 1)) 
            sem_logits_resized = np.expand_dims(sem_logits_resized, axis=0)  
            ...
            if num_instances == 0:
                print("     No instances detected. Try:")
            ...
            def generate_distinct_color(inst_id, num_instances):
                import colorsys
                hue = (inst_id * 137.508) % 360  
                saturation = 0.7 + (inst_id % 3) * 0.1  
                value = 0.8 + (inst_id % 2) * 0.15  
            ...
            # Also save as uint16 for full ID range 
