    ...
    if MIXED_PRECISION_AVAILABLE:
        try:
            ...
            if 'float32' not in current_policy.lower():
                mixed_precision.set_global_policy('float32')
                print(f" Warning: Mixed precision policy was {current_policy}, changed to float32")
        except:
            pass
...
def instances_from_sem_and_boundary(sem_logits, boundary_logits, thing_class_ids: List[int],
    ...
    for cid in thing_class_ids:
        ...
        selem_size = max(3, int(np.sqrt(mask.sum()) / 50))  
        ...
        min_distance = max(10, int(np.sqrt(mask.sum()) / 10))  
        coords = peak_local_max(distance, 
                               footprint=np.ones((7,7)),  
                               min_distance=min_distance,  
                               labels=mask,
                               threshold_abs=distance.max() * 0.4)  
    ...
    if MIXED_PRECISION_AVAILABLE:
        try:
            ...
            if 'float32' in current_policy.lower():
                ...
                mixed_precision.set_global_policy('float32')
                print(f"  Warning: Policy was {current_policy}, changed to float32")
        except Exception as e:
            print(f"  Warning: Could not verify mixed precision policy: {e}")
    else:
    ...
    if args.architecture == "unet_backbone":
        ...
        if MIXED_PRECISION_AVAILABLE:
            try:
                ...
                if 'float32' not in current_policy.lower():
                    mixed_precision.set_global_policy('float32')
                    print(f"  Policy was {current_policy}, reset to float32")
                else:
